...
class UserRoutesUnitTest {
    ...
    fun `create update delete user via handle helpers`() {
        ...
        val fetched = jdbi.withHandle<String, Exception> { h ->
            h.createQuery("select name from users where id = :id").bind("id", created.user.userId).mapTo<String>().one()
        }
        ...
        val count = jdbi.withHandle<Int, Exception> { h ->
            h.createQuery("select count(*) from users where id = :id").bind("id", created.user.userId).mapTo<Int>().one()
        }
